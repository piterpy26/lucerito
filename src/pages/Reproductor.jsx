...
const themes = {
  dark: {
    ...
    favInactiveText: "#b0998a",
    favInactiveBorder: "#4a3e30",
  },
  light: {
    bg: "#f2ece3",
    nav: "#e8dfd3",
    ...
    favInactiveBg: "transparent",
    favInactiveText: "#7a6355",
    favInactiveBorder: "#c8bfb2",
  },
};
...
const HomeButton = memo(function HomeButton({ theme, isDark, onClick }) {
  return (
    <button
      onClick={onClick}
      className="fixed top-3 right-3 z-50 flex items-center gap-1 backdrop-blur-md border text-xs font-semibold px-3 py-2 rounded-xl hover:opacity-80 active:scale-95 transition-all duration-200"
      style={{
        backgroundColor: isDark ? "rgba(255,255,255,0.10)" : "rgba(0,0,0,0.07)",
        borderColor: isDark ? "rgba(255,255,255,0.18)" : theme.border,
        color: isDark ? "#f5f0ea" : theme.text,
        boxShadow: isDark
...
const ProgressBar = memo(function ProgressBar({
  ...
  const handleChange = useCallback(
    (e) => onSeek(Number(e.target.value)),
    [onSeek],
  );
  return (
        ...
        {isDragging && (
          <motion.div
            ...
            style={{
              ...
              boxShadow: `0 0 12px ${theme.light}`,
            }}
          />
        )}
        <input
    ...
    <div className="flex items-center gap-2 w-full">
      ...
      <div className="flex-1 relative h-7 flex items-center cursor-pointer">
        ...
        >
          <div
            className="h-full rounded-full"
            style={{ width: `${volume * 100}%`, backgroundColor: theme.light }}
          />
        </div>
        {isDragging && (
          <div
...
const SongRow = memo(function SongRow({
  ...
  return (
    <div
      ...
      className="flex items-center gap-2.5 px-2 py-2 rounded-2xl cursor-pointer outline-none select-none transition-colors duration-150"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p
          ...
          style={{ color: isActive ? theme.light : theme.text }}
        >
          {song.title}
        </p>
      ...
      <div className="flex items-center shrink-0" style={{ gap: "2px" }}>
        <motion.button
          ...
          className="w-8 h-8 rounded-full flex items-center justify-center border-none bg-transparent cursor-pointer"
        >
          {isFav ? (
            <RiHeart3Fill size={18} style={{ color: theme.light }} />
          ) : (
            <RiHeart3Line size={18} style={{ color: theme.accent }} />
          )}
        </motion.button>
        {onEdit && (
          <motion.button
      ...
      <div className="max-w-lg mx-auto">
        ...
        >
          ...
          <motion.button
            ...
            style={{
              backgroundColor: theme.accent,
              color: isDark ? theme.light : theme.card,
            }}
          >
...
const TAB_ITEMS = [
  {
    ...
    IdleIcon: RiHome4Line,
    label: "Principal",
  },
  {
    key: "library",
    ActiveIcon: RiMusic2Fill,
    IdleIcon: RiMusic2Line,
    label: "Biblioteca",
  },
  {
    key: "favorites",
    ActiveIcon: RiHeart3Fill,
    IdleIcon: RiHeart3Line,
    label: "Favoritos",
  },
];
      ...
      {TAB_ITEMS.map(({ key, ActiveIcon, IdleIcon, label }) => {
        ...
        return (
          ...
          >
            <Icon
              size={24}
              style={{ color: active ? theme.light : theme.accent }}
            />
            <span
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-center justify-evenly px-8 max-w-lg mx-auto w-full">
        ...
        <div className="w-full text-center">
          ...
          <p className="text-base truncate mt-1" style={{ color: theme.muted }}>
            {song?.artist || ""}
          </p>
        </div>
        <div className="w-full">
        ...
        {list.length === 0 ? (
          ...
          list.map((s) => (
            <SongRow
              key={s.id}
              song={s}
              isActive={song?.id === s.id}
              playing={playing}
              isFav={favorites.includes(s.id)}
              onPlay={() => play(songs.indexOf(s))}
              onToggleFav={() => toggleFav(s.id)}
              onEdit={showActions ? handleEditSong : null}
              onDelete={showActions ? handleDeleteSong : null}
              theme={theme}
            />
          ))
        )}
